    ...
    def main(self):
        ...
        while True:
            ...
            if mode == mode_list.index("numpad_mode"):
                ...
                output = self.alpha(rotary_dial_number)
                ...
                output = self.alfred(rotary_dial_number)
            elif mode == mode_list.index("backspace_mode"):
                self.backspace(rotary_dial_number)
                mode = previous_mode
                logging.info("Switched back to %s", mode_list[mode])
                output = ""
            ...
            if mode == mode_list.index("alfred_mode"):
                self.send_Alfred_Hotkey(output)
            elif shift or capslock:
                self.send_shifted_key(mode, output)
                shift = False
            else:
...
if __name__ == "__main__":
    app = rumps.App("QRD Statusbar", title="QRD", menu=["Quit All"], quit_button=None)
    # app = QRDStatusBarApp()
    app.run()
